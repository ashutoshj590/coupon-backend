...
var s3Bucket = new AWS.S3( { params: {Bucket: 'my-coupon'} } );
...
exports.sendEmail = function(to,  subject, template, templateData) {
   ...
   // var from = "Coupon Support <ashutoshj590@gmail.com>";
    to = [to];
    getEmailBody(templateBody, templateData, function(emailBody){
        ses.sendEmail({
                Source: "ashutoshj590@gmail.com",
...
exports.uploadImageToS3 = function(base64Data, user_id, document_type, folder){
    if(!util.validField(folder))
        folder="";
    var deferred = Q.defer();
    var key = new Date().getTime()+"_"+user_id+"_"+document_type+".jpeg";
